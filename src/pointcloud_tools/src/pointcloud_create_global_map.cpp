...
public:
  ...
  pcl::PointCloud<pcl::PointXYZI> pcl_global_cloud;
  int count ;
  ...
  tf_listener()
{
  ...
  advertise_cloud_default = "global_cloud";
  ...
  velodyne_pub = nh.advertise<sensor_msgs::PointCloud2>( advertise_cloud, 100, true);
}
  ...
  *in_cloud = *msg;
  ...
  pcl::fromROSMsg( *in_cloud, *pcl_PointCloud_ );
	...
	if ( out_frame.compare( in_cloud->header.frame_id) != 0){
	  ...
	  try{
	    tf_listener.waitForTransform( out_frame, in_cloud->header.frame_id, in_cloud->header.stamp, ros::Duration(0.2));
	...
	if ( count >= 5 ){
	  pcl_global_cloud += *pcl_PointCloud_;
	  count =0;
	}
	else
	  count ++;
	...
	pcl::toROSMsg( pcl_global_cloud, *out_cloud );
...
int main(int argc, char **argv){
  ros::init(argc, argv, "pointcloud_create_global_map");	
  Transform node;
