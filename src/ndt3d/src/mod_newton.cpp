...
extern double filtered_scan_points_weight[];
extern double filtered_scan_points_totalweight;
...
extern double hessian[6][6];
extern bool use_voxel_grid_filter;
...
double adjust3d(PointPtr scan, int num, PosturePtr initial,int target){
  ...
  /*点列について繰り返し計算*/
	...
	if (!use_voxel_grid_filter)
		...
		weight_total=filtered_scan_points_totalweight;
  weight_next=0;
  weight_sum=0;
  ...
  //point_fp=fopen("/tmp/range","w");
	for (i = 0; i < num; i++) {
		if (!use_voxel_grid_filter)
			...
			weight_sum+=filtered_scan_points_weight[i];
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 6; j++)  hessian[i][j] = Hsumh[i][j];
  }
