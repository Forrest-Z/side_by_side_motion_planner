...
class AmclNode
{
  ...
  private:
    ...
    void mapUpdateReceived(const map_msgs::OccupancyGridUpdateConstPtr& msg);

    void handleMapMessage(const nav_msgs::OccupancyGrid& msg);
    void handleMapMessage(const map_msgs::OccupancyGridUpdate& msg);
    void freeMapDependentMemory();
    map_t* convertMap( const nav_msgs::OccupancyGrid& map_msg );
    void updateMap( const nav_msgs::OccupancyGrid& map_msg, map_t* map);
    void updateMap( const map_msgs::OccupancyGridUpdate& map_msg, map_t* map);
        ...
        first_reconfigure_call_(true)
{
  ...
  if(use_map_topic_) {
    ...
    map_update_sub_ = nh_.subscribe("map_update", 1, &AmclNode::mapUpdateReceived, this);
...
void AmclNode::savePoseToServer()
{
  ...
  if(!std::isfinite(map_pose.getOrigin().x()) ||
		  !std::isfinite(map_pose.getOrigin().y()) ||
		  !std::isfinite(yaw) ||
		  !std::isfinite(last_published_pose.pose.covariance[6*0+0]) ||
		  !std::isfinite(last_published_pose.pose.covariance[6*1+1]) ||
		  !std::isfinite(last_published_pose.pose.covariance[6*5+5])) return;
...
void
AmclNode::mapUpdateReceived(const map_msgs::OccupancyGridUpdateConstPtr& msg)
{
...
void
AmclNode::handleMapMessage(const map_msgs::OccupancyGridUpdate& msg)
{
  ROS_INFO("Received a %d X %d update\n",
           msg.width,
           msg.height);

  if(first_map_received_)
  {
	  ROS_INFO("AMCL: partially update map");
	  updateMap(msg, map_);
  }
}
...
void AmclNode::updateMap( const map_msgs::OccupancyGridUpdate& map_msg, map_t* map)
{
  int i = 0;
  for(int y=map_msg.y; y<map_msg.y+(int)map_msg.height; y++)
  {
    int j = y * map->size_x + map_msg.x;
    for(int x=map_msg.x; x<map_msg.x+(int)map_msg.width; x++)
    {
      if(map_msg.data[i] == 0)
        map->cells[j].occ_state = -1;
      else if(map_msg.data[i] == 100)
        map->cells[j].occ_state = +1;
      else
        map->cells[j].occ_state = 0;
      j ++;
      i ++;
    }
  }
...
void AmclNode::updateMap( const nav_msgs::OccupancyGrid& map_msg, map_t* map)
{
  for(int i=0;i<map->size_x * map->size_y;i++)
  {
    if(map_msg.data[i] == 0)
