...
void LaneList::print( std::vector< std::vector<double> >& laneList )
{
...
void LaneList::copyLaneListToLaneList( std::vector< std::vector<double> >& laneList, std::vector< std::vector<double> >& returnLaneList )
{  
  //input data
...
void LaneList::copyLaneMessageToLaneList( std_msgs::Float64MultiArray& laneMessage, std::vector< std::vector<double> >& returnLaneList )
{  
  returnLaneList.clear();
...
void LaneList::copyLaneListToLaneMessage( std::vector< std::vector<double> >& laneList, std_msgs::Float64MultiArray& returnLaneMessage )
{  
...
void LaneList::convertLaneListFrame( std::vector< std::vector<double> >& laneList1, std::vector< std::vector<double> >& laneList2, 
				     std::string& frame1Name, std::string& frame2Name )
{
  for( std::vector< std::vector<double> >::iterator itr=laneList1.begin(); itr != laneList1.end(); itr++ ){    
    //point.x = (*itr)[1];
    //point.y = (*itr)[2];    
...
void LaneList::drawLane( std::vector< std::vector<double> >& laneList, std::string& baseTfName, visualization_msgs::Marker& lineStrip )//x0,y0; x1,y1;... 
{
  ...
  for( std::vector< std::vector<double> >::iterator itr=laneList.begin(); itr != laneList.end(); itr++ ){
    ...
    if( itr->size() < 3 ){//expecting x,y,s data
      return;
    }
    point.x = (*itr)[0];
    point.y = (*itr)[1];
  ...
  //_targetLaneInterpBaseMarkPublisher.publish(lineStrip);
}

//xyList x0 y0; x1 y1; ... 
//xyInterpList x0 y0; x1' y1'; ... 
void LaneList::interpXYList( std::vector< std::vector<double> >& xyList, 
			     std::vector< std::vector<double> >& xyInterpList, double interpTick )
{  
  xyInterpList.clear();
  
  //LaneList::intepSpline(xyList, xyInterpList, _interpTick);
