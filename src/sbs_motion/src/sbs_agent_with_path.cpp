...
void sbs_agent::pathCallback(const trajectory_tracker::TrajectoryPath::ConstPtr& input)
{
  std::cerr << "\n pathcallback: " << ns.c_str() << "  right_side_path: " << right_side_path << "\n";
  trajectory_tracker::TrajectoryPath pathMessage = *input;
  ...
  path_rviz.header = pathMessage.header; 
  
  //HERE
  std::ofstream out_file;
  out_file.open ( "/home/yoichims/Dropbox/Research/whill_side_by_side/path.txt" ) ;
  bool init=false;
  
  geometry_msgs::PoseStamped temp;
  geometry_msgs::Point prev, current, vec, normal;
  double cosangle, sinangle;
  self_LaneMessage.data.clear();
  
  for( int itr=0; itr < pathMessage.poses.size() ; itr++ ){
    if (init){
      current = point2d( pathMessage.poses[itr].pose.position.x, pathMessage.poses[itr].pose.position.y) ;

      //Get vector between 2 points in trajectory
      vec = point2d( current.x - prev.x, current.y-prev.y);
      cosangle = cos( atan2(vec.x, -vec.y));
      sinangle = sin( atan2(vec.x, -vec.y));

      //Compute vector normal to trajectory multiplied by preferred_distance / 2
      if( right_side_path ){
	normal = point2d ( -cosangle*social_distance/2 + current.x ,  -sinangle*social_distance/2 + current.y );
	self_LaneMessage.data.push_back( normal.x );
	self_LaneMessage.data.push_back( normal.y );
	temp.pose.position.x = normal.x ;
	temp.pose.position.y = normal.y ;
	
	normal = point2d ( (cosangle*social_distance/2 + current.x) , (sinangle*social_distance/2 + current.y) );
	partner_LaneMessage.data.push_back( normal.x );
	partner_LaneMessage.data.push_back( normal.y );

	//if( strcmp ( ns.c_str(), "v2" ) == 0 )
	//std::cerr << "\n agent " << ns.c_str() << " " << normal.x << " " << normal.y << " ";
      }

      //out_file << current.x << " " << current.y << " "; 
      //out_file << normal.x << " " << normal.y << " ";
      else{
	normal = point2d ( (cosangle*social_distance/2 + current.x) , (sinangle*social_distance/2 + current.y) );
	self_LaneMessage.data.push_back( normal.x );
	self_LaneMessage.data.push_back( normal.y );
	...
	//if( strcmp ( ns.c_str(), "v1" ) == 0 )
      ...
      //out_file << normal.x << " " << normal.y << " \n";
      //out_file << -cosangle + current.x << " " << -sinangle + current.y << " \n";


      //self_LaneMessage.data.push_back( normal.x );
      //self_LaneMessage.data.push_back( normal.y );
      self_LaneMessage.data.push_back(pathMessage.poses[itr].pose.position.z);
      self_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.x);
      self_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.y);
      self_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.z);
      self_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.w);

      partner_LaneMessage.data.push_back(pathMessage.poses[itr].pose.position.z);
      partner_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.x);
      partner_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.y);
      partner_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.z);
      partner_LaneMessage.data.push_back(pathMessage.poses[itr].pose.orientation.w);



      path_rviz.poses.push_back (temp) ;
    }
    
    else
      init=true;
    
    prev = point2d( pathMessage.poses[itr].pose.position.x, pathMessage.poses[itr].pose.position.y);
    //cos(atan(4/3))
  }
  //out_file.close();
  //std::cerr << -cosangle + current.x << " " << -sinangle + current.y << " \n";
  //ros::shutdown();

  //path_rviz.pose =  
  agentglobalpathPublisher.publish(path_rviz);
  

    //copy to message
  std_msgs::MultiArrayDimension dimTmp;
  self_LaneMessage.layout.dim.clear();
  self_LaneMessage.layout.dim.push_back(dimTmp);
  self_LaneMessage.layout.dim.push_back(dimTmp);
  partner_LaneMessage.layout.dim.clear();
  partner_LaneMessage.layout.dim.push_back(dimTmp);
  partner_LaneMessage.layout.dim.push_back(dimTmp);
  
  int nCol = 7;//xyz,xyzw
  int nRow = pathMessage.poses.size();
  
  self_LaneMessage.layout.dim[0].label  = "row";
  self_LaneMessage.layout.dim[0].size   = nRow;
  self_LaneMessage.layout.dim[0].stride = nCol*nRow;
  self_LaneMessage.layout.dim[1].label  = "column";
  self_LaneMessage.layout.dim[1].size   = nCol;
  self_LaneMessage.layout.dim[1].stride = nCol;

  partner_LaneMessage.layout.dim[0].label  = "row";
  partner_LaneMessage.layout.dim[0].size   = nRow;
  partner_LaneMessage.layout.dim[0].stride = nCol*nRow;
  partner_LaneMessage.layout.dim[1].label  = "column";
  partner_LaneMessage.layout.dim[1].size   = nCol;
  partner_LaneMessage.layout.dim[1].stride = nCol;
  
  


  self_LaneList.clear();
  partner_LaneList.clear();
  
  int dimSize=self_LaneMessage.layout.dim.size();
  
  if( dimSize > 1 ){
    int nRow=self_LaneMessage.layout.dim[0].size;
    int nCol=self_LaneMessage.layout.dim[1].size;    
    
    for( int i=0; i < nRow - 1; i++ ){//row
      std::vector<double> selfVector;
      std::vector<double> partnerVector; 
      for( int j=0; j < nCol; j++ ){//column
	selfVector.push_back(self_LaneMessage.data[ nCol*i + j ]);
	partnerVector.push_back(partner_LaneMessage.data[ nCol*i + j ]);
      }
      self_LaneList.push_back(selfVector);
      partner_LaneList.push_back(partnerVector);
    }
  }
  
  //copyTrajectoryPathMessageToLaneMessage( path, _laneMessage );
}
  ...
  pdw(0)
{
  ...
  np.param<bool>( "right_side_path", right_side_path, true );
  np.param<double>( "distance_to_subgoal_ahead", distance_to_subgoal_ahead, 2.2 );
  np.param<double>( "adjust_path_speed_constant", adjust_path_speed_constant, 1.0 );
    
  np.param<bool>( "use_collision", use_collision, false );
  ...
  social_distance = relativeDistC; // /1.2 ;
 
  std::cerr << "\n relative_distance: " << social_distance ;
  ...
  if( strcmp ( ns.c_str(), "v2" ) == 0 )
    std::cerr << "\n Initializing the partner" ;


  pathSub=nh.subscribe("sbs_path", 1000, &sbs_agent::pathCallback,this);
  ...
  trajectoryFilter1MarkPublisher = nh.advertise<visualization_msgs::Marker>("trajectory_mark_filter1", 1);
  trajectoryFilter2MarkPublisher = nh.advertise<visualization_msgs::Marker>("trajectory_mark_filter2", 1);
  trajectoryFilter3MarkPublisher = nh.advertise<visualization_msgs::Marker>("trajectory_mark_filter3", 1);
  optimalTrajectoryMarkPublisher = nh.advertise<visualization_msgs::Marker>("trajectory_mark_optimal", 1);
  optimalTrajectoryPublisher     = nh.advertise<trajectory_tracker::TrajectoryPath>("trajectory_tracker/path", 10);

  agentglobalpathPublisher     = nh.advertise<nav_msgs::Path>("agent_path", 10); 
...
bool sbs_agent::generateTrajectories(std::vector< std::vector<double> > LaneList,
				     std::vector< std::vector< std::vector<double> > > trajectoryArrayCartesianList[3], std::vector< std::vector< std::vector<double> > > previousOptimalTrajectoryArrayCartesianList, double yaw, Eigen::Vector2d currentPos, double dv, double dw, Eigen::Vector2d &next_subgoal, bool self)
{

  /*
  if( self_LaneList.size() <= 0 ){
      ROS_ERROR( "computeTrajectoryArray: targetLaneListInterpVehicle.size() must be larger than 0" );
  ...
  */
  ...
  //init frene frame
  //std::vector< std::vector<double> > self_spline_xyResamplingList;
  //Frene frene(projectedPath, 0.3, xyResamplingList);

  spline_struct splineX, splineY;
  //spline_struct partner_splineX, partner_splineY;
  
  //Frene frene( LaneList, 0.3, /*self_spline_xyResamplingList,*/ splineX, splineY, ns.c_str(), self);
  //Frene frene_partner( partner_LaneList, 0.3, partner_splineX, partner_splineY, ns.c_str(), false );
  //SEEMS to be OK UNTIL HERE  !!  _splineX and _splineY

  Frene frene( LaneList, 0.3, splineX, splineY, ns.c_str(), self);
  ...
  */
  frene.convertFromCartesianState(0,currentPos.x(),currentPos.y(),yaw,dv,0,dw/dv,dw,initialStateFromSensor,arcLengthTickMeter, splineX, splineY, next_subgoal, distance_to_subgoal_ahead, ns.c_str(), self );  
    initialState = initialStateFromSensor;

    //std::cerr << "\n In else";
    // }
    //std::cerr << "\ncurrent: " << currentPos.x() << " " << currentPos.y() << "  subgoal: " << next_subgoal.x() << " " << next_subgoal(1) ;
  ...
  frene.convertMergedTrajectoryArrayToCartesian( trajectoryArrayFreneList, trajectoryArrayCartesianList[0], splineX, splineY );

  

  //if (use_collision==true )
  if( use_collision == true  and  occmap["traversability"].any()){
  ...
  //if( strcmp ( ns.c_str(), "v1" ) == 0 )
  //std::cerr<<"Trajectories input: "<< trajectoryArrayCartesianList[0].size()<<"   out: "<< trajectoryArrayCartesianList[1].size()  << std::endl;
  ...
  /*
  std::vector< std::vector< std::vector<double> > >::iterator itr0 = trajectoryArrayCartesianList[0].begin(); 
  std::vector< std::vector<double> >::iterator it0 = itr0->begin();

  std::cerr << "\n ns: " << ns.c_str() ;
  if( strcmp ( ns.c_str(), "v1" ) == 0 )
    if( self == true )
      //for( std::vector< std::vector<double> >::iterator it0 = itr0->begin(); it0 != itr0->end(); ++it0)
	std::cerr << "\n"<< id <<"  [0] " << (*it0)[0] << " " << (*it0)[1] ;

  
  std::vector< std::vector< std::vector<double> > >::iterator itr1 = trajectoryArrayCartesianList[1].begin(); 
  std::vector< std::vector<double> >::iterator it1 = itr1->begin();
  //if( id ==1)
  if( strcmp ( ns.c_str(), "v1" ) == 0 )
  if( self == true )
    //for( std::vector< std::vector<double> >::iterator it1 = itr1->begin(); it1 != itr1->end(); ++it1)
      std::cerr << "\n"<< id << "  [1] " << (*it1)[0] << " " << (*it1)[1] ;
  */
...
void sbs_agent::copyTrajectoryArrayListToTrajectoryPathMessage( std::vector< std::vector< std::vector<double> > >& laneList, trajectory_tracker::TrajectoryPath& returnPathMessage)
{
  ...
  for( std::vector< std::vector< std::vector<double> > >::iterator itr0=laneList.begin(); itr0 != laneList.end(); itr0++ ){            
	...
	isFirstFlag = false;
      ...
      //Velocity multiplied by Luis  Path_velocity_normalization
      message.twist.linear.x = vx * adjust_path_speed_constant; //0.31 ;
...
void sbs_agent::run(){
  ...
  static std::vector< std::vector< std::vector<double> > > p_previousOptimalTrajectoryArrayCartesianList;
  
  std::vector< std::vector< std::vector<double> > > trajectoryArrayCartesianList[5];
  double max, min;
  Eigen::Vector2d    self_next_subgoal;
  Eigen::Vector2d partner_next_subgoal; 
  ...
  while(nh.ok()){
    ...
    if( self_LaneList.size() <= 0 ){
      //ROS_ERROR( "computeTrajectoryArray: targetLaneListInterpVehicle.size() must be larger than 0" );
      ROS_WARN_ONCE( "frene planner is waiting target path !!!..." );
      //std::cerr << "\n no path \n";
	...
	for (grid_map::GridMapIterator iterator(sub);!iterator.isPastEnd(); ++iterator) {
      ...
      if(!generateTrajectories(self_LaneList, trajectoryArrayCartesianList, previousOptimalTrajectoryArrayCartesianList, yaw, currentPos, dv, dw, self_next_subgoal,true)){
	ROS_ERROR("%s: Error generating self trajectories",ns.c_str());
	twist_pub.publish(stop);
	continue;
      }

      //std::cerr << "\nGenerateTraj. current: " << currentPos.x() << " " << currentPos.y() << "  subgoal: " << self_next_subgoal.x() << " " << self_next_subgoal(1) ;
	...
	continue;
      ...
      std::vector< std::vector< std::vector<double> > > p_optimalTrajectoryArrayCartesianList;


      //double yaw=util.angle(tf::getYaw(currentPose.getRotation()));
      //if( strcmp ( ns.c_str(), "v1" ) == 0 )
      //std::cerr << "\ncurrent: " << currentPos.x() << " " << currentPos.y() << " " << yaw*180/M_PI;
      
      //Self Utility
      optimalTrajectoryArrayCartesianList=util.trajectoryUtil(trajectoryArrayCartesianList[1],trajectoryArrayCartesianList[3], self_next_subgoal, yaw,objs,max,min, ns );

      /*
      std::vector< std::vector< std::vector<double> > >::iterator itr3 = trajectoryArrayCartesianList[3].begin(); 
      std::vector< std::vector<double> >::iterator it3 = itr3->begin();
      ...
      */
      ...
      std::vector< std::vector< std::vector<double> > > p_TrajectoryArrayCartesianList[5];
      //forPartner
      if(numAgents>1)
	{
	  double pMax, pMin;
	  ...
	  //For the Partner!
	  if(generateTrajectories(partner_LaneList, p_TrajectoryArrayCartesianList, p_previousOptimalTrajectoryArrayCartesianList, pYaw, partnerPos, pdv, pdw, partner_next_subgoal, false))
	    {
	      if(p_TrajectoryArrayCartesianList[1].size()==0)
		p_TrajectoryArrayCartesianList[4]=util.trajectoryStop( trajectoryArrayCartesianList[3], trajectoryArrayCartesianList[4],partnerPos,pYaw);
	      else {
		
		util.trajectoryUtil(p_TrajectoryArrayCartesianList[0],p_TrajectoryArrayCartesianList[3],  partner_next_subgoal, pYaw,objs,pMax,pMin, ns);
		
		//p_TrajectoryArrayCartesianList[4]=util.combinedTrajectory(trajectoryArrayCartesianList[3], p_TrajectoryArrayCartesianList[3],trajectoryArrayCartesianList[4],pMax,pMin);
		p_TrajectoryArrayCartesianList[4]=util.LuiscombinedTrajectory(trajectoryArrayCartesianList[3], p_TrajectoryArrayCartesianList[3],trajectoryArrayCartesianList[4],pMax,pMin,currentPos, yaw, partnerPos, pYaw );     
	      }
	      TrajectoryArray::copyTrajectoryArrayListToTrajectoryArrayList(p_TrajectoryArrayCartesianList[4], p_optimalTrajectoryArrayCartesianList);
      ...
      /*
      std::vector< std::vector< std::vector<double> > >::iterator itr31 = trajectoryArrayCartesianList[3].begin(); 
      std::vector< std::vector<double> >::iterator it31 = itr31->begin();
      if( strcmp ( ns.c_str(), "v1" ) == 0 )
	std::cerr << "\n"<< id << "  [3.2] " << (*it31)[0] << " " << (*it31)[1] ;
        
      std::vector< std::vector< std::vector<double> > >::iterator itr4 = trajectoryArrayCartesianList[4].begin(); 
      std::vector< std::vector<double> >::iterator it4 = itr4->begin();
	...
	std::cerr << "\n"<< id << "  [4] " << (*it4)[0] << " " << (*it4)[1] ;
      */
      ...
      //drawTrajectory( p_TrajectoryArrayCartesianList[3], trajectoryFilter2MarkPublisher, 0.5, 0.5, 0.0, 0, visualization_msgs::Marker::LINE_LIST,max,min,false );//trajectories
      ...
      TrajectoryArray::copyTrajectoryArrayListToTrajectoryArrayList(p_optimalTrajectoryArrayCartesianList,p_previousOptimalTrajectoryArrayCartesianList);
