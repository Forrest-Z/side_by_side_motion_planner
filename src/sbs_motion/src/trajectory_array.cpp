...
void TrajectoryArray::print( std::vector<std::vector< std::vector<double> > >& trajectoryArrayList )
{
  for( std::vector< std::vector< std::vector<double> > >::iterator itr1=trajectoryArrayList.begin(); itr1 != trajectoryArrayList.end(); itr1++ ){            
...
void TrajectoryArray::copyTrajectoryArrayListToTrajectoryArrayList( std::vector< std::vector< std::vector<double> > >& laneList, std::vector< std::vector< std::vector<double> > >& returnLaneList )
{  
  //input data
  returnLaneList.clear();
  for( std::vector< std::vector< std::vector<double> > >::iterator itr1=laneList.begin(); itr1 != laneList.end(); itr1++ ){        
...
void TrajectoryArray::copyTrajectoryArrayListToTrajectoryArrayList_extending_last_point( std::vector< std::vector< std::vector<double> > >& laneList, std::vector< std::vector< std::vector<double> > >& returnLaneList )
{  
  //input data
  returnLaneList.clear();
  for( std::vector< std::vector< std::vector<double> > >::iterator itr1=laneList.begin(); itr1 != laneList.end(); itr1++ ){        
...
void TrajectoryArray::copyTrajectoryArrayMessageToLaneList( std_msgs::Float64MultiArray& laneMessage, std::vector< std::vector< std::vector<double> > >& returnLaneList )
{  
  returnLaneList.clear();
...
void TrajectoryArray::copyTrajectoryArrayListToTrajectoryArrayMessage( std::vector< std::vector< std::vector<double> > >& trajectoryArrayList, 
...
void TrajectoryArray::mergeTrajectoryList( std::vector< std::vector<double> >& t1, 
					   ...
					   std::vector< std::vector<double> >& tMerge )
{
  ...
  if( nRow1 != nRow2 || nRow1 < 0 ){
    std::cout << "Error. TrajectoryArray::mergeTrajectoryArrayList" << std::endl;
    return;
  }
  ...
  for( int i=0; i < nRow1; i++ ){
    ...
    //tMerge[i].insert( tMerge[i].end(), t1[i].begin(), t1[i].end() );
    //tMerge[i].insert( tMerge[i].end(), t2[i].begin(), t2[i].end() );
  }
}


/*
void TrajectoryArray::mergeTrajectoryArrayMessage( std_msgs::Float64MultiArray& t1, std_msgs::Float64MultiArray& t2, std_msgs::Float64MultiArray& tmerge )
{  
  tmerge.data.clear();
  ...
  if( dimSize > 1 ){
    int nDim=t1.layout.dim[0].size;
    int nRow=t1.layout.dim[1].size;
    int nCol=t1.layout.dim[2].size;    
    
    for( int i=0; i < nDim; i++ ){//dim
      for( int j=0; j < nRow; j++ ){//row
	std::vector<double> tmpVector; 
	for( int k=0; k < nCol; k++ ){//column
	  
	  double v1 = t1.data[  nRow*nCol*i + nCol*j + k ];
	  
	  tmpVector.push_back( t1.data[  nRow*nCol*i + nCol*j + k ]);
	}
