...
void GeneratePolyTrajectory::solveLinearEquations(Eigen::MatrixXd& A, Eigen::VectorXd& b, Eigen::VectorXd& x)
{
  ...
  if(A.isApprox(static_cast<Eigen::MatrixXd>(A.triangularView<Eigen::Lower>()),absDiff)){ // A Is Lower Triangular
    x = A.triangularView<Eigen::Lower>().solve(b);           // This is a simple forward substitution on b
  }
  else if( A.isApprox(static_cast<Eigen::MatrixXd>(A.triangularView<Eigen::Upper>()),absDiff)){  // A is upper triangular
     x = A.triangularView<Eigen::Upper>().solve(b);           // This is a simple backward substitution on b      
  } 
  else{
    if( A.isApprox(A.transpose(),absDiff)){ // A is symmetric
	//Eigen::MatrixXd R = A.llt().MatrixU();
	Eigen::LLT<Eigen::MatrixXd> lltOfA(A);      
	if (lltOfA.info()==Eigen::NumericalIssue){           // A is symmetric positive definite
    ...
    Eigen::VectorXd y = static_cast<Eigen::MatrixXd>(lu.matrixLU().triangularView<Eigen::UnitLower>()).colPivHouseholderQr().solve(lu.permutationP()*b); //lu.permutationP() may need to be transposed
    x = static_cast<Eigen::MatrixXd>(lu.matrixLU().triangularView<Eigen::Upper>()).colPivHouseholderQr().solve(y); // a forward and a backward substitution    
  }
...
void GeneratePolyTrajectory::startComputeQuarticPolynomial(double s0, double dts0, double ddts0, 
							   double tf, double dtsf, double ddtsf, 							   
...

{
  ...
  for( double t=0.0; t < tf; t+= tickTime ){
    ...
    //std::cout << t << " " << xxt << " " << vvt << " " << aat << " " << jjt << std::endl; 
...
void GeneratePolyTrajectory::startComputeQuarticPolynomialWithFinalPosition(
									    double s0, double dts0, double ddts0, 
									    double tf, double sf, double dtsf, 							   
									    std::vector< std::vector<double> >& resultTrajectory,
									    std::vector< std::vector<double> >& resultParam,
									    double tickTime
									    )

{
  ...
  //define b vector
  Eigen::VectorXd b(2);
  double tmp1 = vvf - (cc2*tf + cc3);
  double tmp2 = xxf - (1/2.*cc2*tf2+cc3*tf+cc4);
  b << tmp1, tmp2;
...
void GeneratePolyTrajectory::startComputeQuinticPolynomial(double d0, double dtd0, double ddtd0, 
							   double tf, double df, double dtdf, double ddtdf, 							   
							   std::vector< std::vector<double> >& resultTrajectory,
							   std::vector< std::vector<double> >& resultParam,
							   double tickTime
							   )
{
  ...
  GeneratePolyTrajectory::startComputeQuinticPolynomialWithAmplitude(d0, dtd0, ddtd0, 
								     tf, df, dtdf, ddtdf, 							   
								     resultTrajectory,
								     resultParam,
								     tickTime,
...
void GeneratePolyTrajectory::startComputeQuinticPolynomialWithAmplitude(double d0, double dtd0, double ddtd0, 
									double tf, double df, double dtdf, double ddtdf, 							   
							   std::vector< std::vector<double> >& resultTrajectory,
							   std::vector< std::vector<double> >& resultParam,
							   double tickTime,
							   ...
							   )
{
  ...
  A <<   1/6.*tf3 ,  1/2.*tf2,      tf ,
    1/24.*tf4 ,  1/6.*tf3 , 1/2.*tf2,
    1/120.*tf5 , 1/24.*tf4 , 1/6.*tf3;
...
void GeneratePolyTrajectory::computeQuinticValueFromParam( double t, 
							   double cc0, double cc1, double cc2, double cc3, double cc4, double cc5, 
...
void GeneratePolyTrajectory::computeQuarticValueFromParam( double t, 
							   double cc0, double cc1, double cc2, double cc3, double cc4, 
							   double& xxt, double& vvt, double& aat, double& jjt )
{
  double t2=t*t;
  double t3=t2*t;
